...
def edit_source(
    ...
    return pampy.match(
        ...
        lambda edit: token_info_for_insert(
            snapshot, start=edit["offset"], text=edit["text"], next_id=next_id, id=id
        ),
        {"type": "delete"},
        lambda edit: token_info_for_delete(
            snapshot, start=edit["offset"], size=edit["len"], next_id=next_id, id=id
        ),
    )
...
def token_info_for_delete(
    old_snapshot: Snapshot, start: int, size: int, next_id=Id(), id=0,
):
...
def token_info_for_insert(
    old_snapshot: Snapshot, start: int, text: str, next_id=Id(), id=0,
):
...
def assign_tokens_to_gazes(
    ...
    for gaze in gazes:
        if gaze[gaze_config.time_key] > snapshots[next_edit_i].time:
            # TODO add assertion here
            current_version += 1
            if next_edit_i + 1 < len(snapshots):
                next_edit_i += 1
        line, col = gaze[gaze_config.line_key], gaze[gaze_config.col_key]
        snapshot = snapshots[current_version]

        index = snapshot.source.mapping[line, col]
        token = token_at_index(snapshot, index)  # TODO
        ...
        if next_edit_i - 1 < 0:
            ...
            gaze["last_edit"] = snapshots[next_edit_i - 1].time
